...
func TestReceiptEndToEnd(t *testing.T) {
	...
	if !validReportedScale(scale) {
		t.Fatalf("renderer scale = %v, want 0.79", scale)
	}
...
func TestValidReportedScaleAcceptsOnlyFixedScale(t *testing.T) {
	for _, scale := range []float64{0.50, 0.789999, 0.790001, 1.0} {
		if validReportedScale(scale) {
			t.Fatalf("validReportedScale(%v) = true, want false", scale)
		}
	}
	if !validReportedScale(0.79) {
		t.Fatal("validReportedScale(0.79) = false, want true")
	}
}

func validReportedScale(scale float64) bool {
	return !math.IsNaN(scale) && !math.IsInf(scale, 0) && scale == 0.79
}
