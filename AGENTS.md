...
- Rendering embeds the two approved Google Play logo PNGs, removes other remote images, preserves all `#EDEDED` styling, blocks all HTTP(S) requests, and uses no proxy.
- Every PDF page must be A5 portrait. Rendering uses a fixed `0.79` scale and generates as many pages as content requires. CSS owns the 2 mm page margin, leaving a 144 x 206 mm printable area, so CDP PDF margins remain zero.
...
- `receipt.eml` is the end-to-end input, `receipt.html` is the decoded-body reference, and `receipt.pdf` is the visual reference. They contain real-looking receipt/account data; never print, log, quote, commit, or expose their contents.
